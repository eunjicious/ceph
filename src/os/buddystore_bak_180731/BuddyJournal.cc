...
void BuddyJournal::queue_completions_thru(uint64_t seq)
{
  ...
  dout(10) << __func__ << " completions_item = " << items.size() << dendl;
  while (it != items.end()) {
    ...
    if (next.finish){
      dout(10) << __func__ << " finisher_queue seq = " << seq << dendl;
...
//      finisher->queue(next.finish);
  ...
  dout(10) << __func__ << " wake up finisher " << dendl;
...
void BuddyJournal::do_write(bufferlist& bl)
{
  ...
  dout(10) << "do_write latency " << lat << dendl;
  ...
  } /// finisher lock is unlocked 
//  dout(10) << __func__ << " finisher_wait_for_empty start .." << dendl;
//  finisher->wait_for_empty();
//  dout(10) << __func__ << " finisher_wait_for_empty end .." << dendl;
...
//  dout(10) << "prepare_entry " << tls << dendl;
  ...
  for (vector<ObjectStore::Transaction>::iterator p = tls.begin();
   ...
   if ((int)(*p).get_data_length() > data_len) {
    ...
    ::encode(*p, bl); // 아예 transaction 을 encoding 해버림. 
#endif
			       ...
			       Context *oncommit, TrackedOpRef osd_op)
{
...

  {
    ...
    dout(10) << "completions.push_back seq = " << seq << dendl;
    if (writeq.empty())
      writeq_cond.Signal();
