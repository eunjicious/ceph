...
#ifndef CEPH_BDJOURNALINGOBJECTSTORE_H
#define CEPH_BDJOURNALINGOBJECTSTORE_H
...
protected:
  ...
  class ApplyManager {
    ...
    uint64_t max_applied_seq_thru; // EUNJI: all ops smaller than this have been applied 
    set<uint64_t> early_applied_seq;
  ...
  public:
    ...
    void init_seq(uint64_t fs_op_seq) {
	  ...
	  }
      {
		...
		max_applied_seq_thru = fs_op_seq;
	  }
    }
  } apply_manager;
...
public:
  BDJournalingObjectStore(CephContext* cct, const std::string& path)
    : ObjectStore(cct, path),
      journal(NULL),
      finisher(cct, "BDJObjStore", "fn_bdj_objstore"),
      submit_manager(cct),
