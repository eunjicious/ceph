...
//  int fd;
  ...
  explicit buddy_iov_t(const coll_t _cid, const ghobject_t& _oid, string _fn, off_t _src_off, off_t _ooff, off_t _foff, uint64_t _bytes) 
	: cid (_cid), oid(_oid), fname (_fn), ooff(_ooff), foff(_foff), bytes(_bytes), bnum(round_down(foff)), src_off(_src_off)
  {}
  buddy_iov_t() : fname("init"), ooff(0), foff(0), bytes(0), bnum(0) {}
  ~buddy_iov_t(){}
