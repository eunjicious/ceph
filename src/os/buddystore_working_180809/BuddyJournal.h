...
#ifndef CEPH_BUDDYJOURNAL_H
#define CEPH_BUDDYJOURNAL_H

#include <deque>
using std::deque;

#include "GenericJournal.h"
...
public:
  ...
  //int prepare_entry(bufferlist* tbl) override;
...
private:
  ...
  class Writer : public Thread {
    BuddyJournal *journal;
  public:
    explicit Writer(BuddyJournal *fj) : journal(fj) {}
    void *entry() override {
  ...
  class WriteFinisher : public Thread {
    BuddyJournal *journal;
  public:
    explicit WriteFinisher(BuddyJournal *fj) : journal(fj) {}
    void *entry() override {
 ...
 public:
  BuddyJournal(CephContext* cct, uuid_d fsid, Finisher *fin, Cond *sync_cond,
	      const char *f, bool data_flush_=false, bool dio=false, bool ai=true, bool faio=false) :
    GenericJournal(cct, fsid, fin, sync_cond),
    finisher_lock("BuddyJournal::finisher_lock", false, true, false, cct),
    journaled_seq(0),
    plug_journal_completions(false),
    writeq_lock("BuddyJournal::writeq_lock", false, true, false, cct),
    completions_lock(
      "BuddyJournal::completions_lock", false, true, false, cct),
    fn(f),
    zero_buf(NULL),
    max_size(0), block_size(0),
    data_flush(data_flush_),
    directio(dio), aio(ai), force_aio(faio),
    must_write_header(false),
    write_pos(0), read_pos(0),
    discard(false),
#ifdef HAVE_LIBAIO
    aio_lock("BuddyJournal::aio_lock"),
    aio_ctx(0),
    ...
    write_lock("BuddyJournal::write_lock", false, true, false, cct),
    write_stop(true),
    aio_stop(true),
    write_thread(this),
    write_finish_thread(this) {
      ...
      if (aio && !directio) {
	lderr(cct) << "BuddyJournal::_open_any: aio not supported without directio; disabling aio" << dendl;
        aio = false;
      }
#ifndef HAVE_LIBAIO
      if (aio) {
	lderr(cct) << "BuddyJournal::_open_any: libaio not compiled in; disabling aio" << dendl;
        aio = false;
      }
#endif
...
WRITE_CLASS_ENCODER(BuddyJournal::header_t)
