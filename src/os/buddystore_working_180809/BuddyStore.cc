...
void BuddyStore::do_force_sync()
{
  ...
  Mutex::Locker l(index_write_lock);

  force_index_write = true;
...
//----------- here: buffered read ------- //
#ifdef BUDDY_READ_BUFFER
      tot_reads += data_file.read(fbl, (*iovp).foff, (*iovp).bytes);
#else
				 ...
				 ThreadPool::TPHandle *handle)
{
  ...
  if (data_flush) {
	// 1. prepare_write 
	fc_prepare_write(o->tls, o->tls_iov);

	// 2. update transaction with data location in file system 
	fc_transaction_map_update (o->tls, o->tls_iov); 
  }
...
int BuddyStore::generate_iov(vector<Transaction>& tls, vector<buddy_iov_t>& tls_iov)
{
  ...
  for (vector<Transaction>::iterator p = tls.begin(); p != tls.end(); ++p) {
	...
	for(vector<Transaction::Op>::iterator op_p = tr.punch_hole_ops.begin();
	  ...
	  if (ret < 0){
		dout(10) << "Failed to alloc oid = " << oid << " off = " << off << " len = " << len << dendl;
		return -ENOENT;
	  }
	  ...
	  // 실제 journal 에 해당 정보를 적을 수 있도록 추가를 해줘야 함. 
...
void BuddyStore::generate_iov_data_bl(vector<buddy_iov_t>& iov, bufferlist& bl, uint32_t start_off)
{
 ...
 // iov 에서 ooff 는  
  for(vector<buddy_iov_t>::iterator ip = iov.begin();
    ...
    dout(10) << __func__ << " start_off " << start_off << " ooff " << (*ip).src_off << " len " << (*ip).bytes << " bl.length() " << bl.length() << dendl;
   
    bufferlist newdata;
  
    // copy data from bl 

	size_t zero_len = foff - round_down(foff);
	newdata.append_zero((*ip).zero_len);
    //newdata.append_zero((*ip).off_in_blk);
    newdata.substr_of(bl, start_off + (*ip).src_off, (*ip).bytes);
    dout(10) << " newdata length " << newdata.length() << dendl;
    //newdata.append_zero((*ip).alloc_bytes - newdata.length());
    newdata.append_zero(round_up((*ip).foff + (*ip).bytes) - newdata.length());
    ...
    assert(round_up((*ip).foff + (*ip).bytes) == (*ip).data_bl.length());
  ...
  const off_t foff, const ssize_t bytes)
{
  ...
  dout(3) << __func__ << " oid " << oid << " ooff " << ooff <<
  ...
  if(omap_p == data_file_index_map.end()){
    dout(3) << __func__ <<  " oid is not found. create index map " << dendl;
  ...
  for (map<off_t, buddy_index_t>::iterator idx_p = omap->index_map.begin();
    ...
    idx_p++){
    dout(3) << __func__ << " oid " << oid << " " << count << ":" << " index: " << idx_p->second << dendl; 
  }

  // 이게 second 에서 뭐가 출력되는거지? 쓰레기인거 같은데   
  sp = omap->index_map.upper_bound(ooff);
  dout(3) << __func__ << " ooff " << ooff << " upper_bound " << sp->second << dendl;

  if(sp == omap->index_map.begin())
    dout(3) << __func__ << " sp = begin " << dendl;
  else if(sp == omap->index_map.end())
    dout(3) << __func__ << " sp = end " << dendl; 

 
  if (sp == omap->index_map.begin() && sp == omap->index_map.end()){
    dout(3) << __func__ << " empty index map " << dendl;
  ...
  while(p != omap->index_map.end()){
    ...
    dout(3) << __func__ << " p iov " << p->second << dendl;

    os = p->second.ooff;
    oe = p->second.ooff + p->second.bytes - 1;
    dout(3) << __func__ << " os " << os << " oe " << oe << dendl;
    dout(3) << __func__ << " ns " << ns << " ne " << ne << dendl;
    ...
    dout(3) << __func__ << " prev_idx " << prev_idx << " post_idx " << post_idx << dendl;
...
//      omap->index_map.insert(make_pair(prev_idx.ooff, prev_idx));
    ...
    if(ne > os && ne < oe) {
      ...
      dout(3) << __func__ << " post_idx " << dendl;
      //omap->index_map.insert(make_pair(post_idx.ooff, post_idx));
    }

    dout(3) << __func__ << " increment p " << dendl;
    p++;

  }
//delete_index:
  dout(3) << __func__ << " delete_list " << delete_list.size() << dendl;

  for(list<off_t>::iterator p = delete_list.begin(); p != delete_list.end() ; p++){
    dout(3) << __func__ << " delete : " << *p << dendl;
    omap->index_map.erase(*p);
  }

insert_partial_index:
  dout(3) << __func__ << " partial_list " << frag_list.size() << dendl;
...
// for debugging... 
  for(map<off_t, buddy_index_t>::iterator tmp = omap->index_map.begin(); 
    ...
    dout(3) << __func__ << " oid " << oid << " index_map = " << (*tmp) << dendl;
  }
  return 0;
}
    ...
    vector<buddy_iov_t>& iov)
{
  ...
  while(rbytes > 0) {
    ...
    buddy_iov_t* niov = new buddy_iov_t(cid, oid, "/mnt/buddystore/data_file.0", soff, ooff, foff, fbytes; 
    iov.push_back(*niov);
...
void BuddyStore::data_write_thread_entry()
{
  ...
  while (!stop_data_write) {
	...
	while(!dataq.empty()){
	  ...
	  dout(10) << __func__<< " item.size = " << items.size() << dendl;
		  ...
		  it++)
	  {
		...
		for(vector<buddy_iov_t>::iterator iovp = o->tls_iov.begin();
		  ...
		  if(r != aggr_iovec.begin()){
			...
			dout(10) << __func__ << " prev_node foff " << r->foff << dendl;

			//if((r->foff + r->alloc_bytes) == iov.foff){
			if(round_up((r->foff + r->bytes)) == iov.foff){
		  ...
		  if(r != aggr_iovec.end()){
			...
			if(r->foff == round_up(iov.foff + iov.bytes)){	
...
////////////////////
  ...
  for(vector<buddy_iov_t>::iterator iovp = o->tls_iov.begin();
	  ...
	  int r = fcon->object_extent_map_update (iov,oid, iov.ooff, iov.foff, iov.bytes, iov.fid);
	  assert(r == 0);
  } 
#endif
  o->tls.clear();
