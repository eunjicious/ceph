...
#ifndef CEPH_GENERIC_JOURNAL_H
#define CEPH_GENERIC_JOURNAL_H
...
public:
  GenericJournal(CephContext* cct, uuid_d f, Finisher *fin, Cond *c=0) :
    fsid(f), finisher(fin), cct(cct), logger(NULL),
    do_sync_cond(c),
    wait_on_full(false) { }
  virtual ~GenericJournal() { }
