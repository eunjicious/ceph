...
int ObjBencher::aio_bench(
  ...
  else if (OP_RAND_REMOVE == operation) {
	// max_objects refers to remove_ratio 
    r = clean_up_partial (num_objects, prevPid, concurrentios, max_objects);
	if (r != 0) goto out; 
  }
...
// EUNJI 
// remove a given portion of objects randomly
// max_objects 가 aio_bench 로 넘어오는데 write 에서만 쓰이고 cleanup 에서는 안쓰임. 
// 여기에 remove_objectss 넣어서 보내도록 하기. random remove 인 경우에. 
int ObjBencher::clean_up_partial(int num_objects, int prevPid, int concurrentios, unsigned remove_ratio) {
  ...
  int remove_objects = num_objects;
  int rand_id;
  std::set<int> occur;

  // EUNJI: num_objects to be removed 
  if (remove_ratio < 100) {
	remove_objects = num_objects * (remove_ratio/100.0);
  }

  // don't start more completions than files
  if (remove_objects < concurrentios) {
    concurrentios = remove_objects;
  }
...
////// EUNJI 
	while(1){
	  rand_id = rand() % num_objects;
	  set<int>::iterator i = occur.find(rand_id);
	  if(i == occur.end()){ // not occur 
		occur.insert(rand_id);
		break;
	  }
	}

    newName = generate_object_name(rand_id, prevPid);
 ...
 ERR:
  out(cout) << "Err: Removed " << data.finished << std::endl;
  lock.Lock();
  data.done = 1;
  lock.Unlock();
  return r;
